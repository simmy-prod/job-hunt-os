...
test("one unavailable source cannot block or corrupt a healthy source's results", async () => {
  ...
  try {
    ...
    assert.deepEqual(northwind, {sourceId: "northwind", ok: true, errorCode: null, fetched: 4, duplicatesInBatch: 1});
    assert.deepEqual(meridian, {sourceId: "meridian", ok: false, errorCode: "SOURCE", fetched: 0, duplicatesInBatch: 0});

    // 1001 (duplicate, last wins) and 1003 normalize; 1002 (no title) is a review.
    assert.deepEqual(run.added.sort(), ["northwind:1001", "northwind:1003"]);
    ...
    assert.deepEqual(store.listListings().map((item) => item.id), ["northwind:1001", "northwind:1003"]);
...
test("a source going down on a later run does not erase what it discovered while healthy", async () => {
  ...
  try {
    ...
    assert.deepEqual(before, ["northwind:1001", "northwind:1003"]);
