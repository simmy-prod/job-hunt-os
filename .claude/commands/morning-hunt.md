...
3. Drill rep (skip on a busy day; steps 1-2 are the floor that must not lapse).
   Pick the lowest-confidence skill from `profile/skills-matrix.md` (or ask which
   company's interview is coming up soonest) and run one `interview-drill` round
   (3-5 questions) in that context.
4. Close with a one-line status: matches found, follow-ups due, drill done (or
   skipped).
